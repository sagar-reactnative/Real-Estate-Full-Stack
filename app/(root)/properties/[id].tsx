...
const Property = () => {
    ...
    const parseFacility = (facilityName: string): { icon: any, title: string } => {
        switch (facilityName) {
            case "Laundry":
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="pb-20 bg-white">
                ...
                <View className="z-50 absolute inset-x-7 top-5">
                    <View className="flex flex-row justify-between">
                        <TouchableOpacity className="flex justify-center items-center bg-primary-200 rounded-full size-11"
                                          onPress={() => router.back()}>
                ...
                <View className="p-5">
                    <View className="mb-5">
                        <Text className="font-rubik-extrabold text-2xl mb-5">{ data.name }</Text>
                        <View className="flex flex-row items-center gap-3 mb-5">
                            <View className="px-4 py-2 bg-primary-200 rounded-full">
                                <Text className="text-xs text-primary-300 font-rubik-medium">{ data.type?.toUpperCase() }</Text>
                            </View>
                            <View className="flex flex-row items-center gap-1">
                                <Image source={icons.star} className="size-5" />
                                <Text className="text-xs">{ data.rating } / 5 ({ data.reviews?.length ?? 0 } reviews)</Text>
                            </View>
                        </View>
                        <View className="flex flex-row items-center justify-between">
                            <View className="flex flex-row justify-center items-center gap-2">
                                ...
                                <Text className="text-sm font-rubik-bold">{ data.bedrooms } Beds</Text>
                            </View>
                            ...
                            <View className="flex flex-row justify-center items-center gap-2">
                                ...
                                <Text className="text-sm font-rubik-bold">{ data.bathrooms } baths</Text>
                            </View>
                            ...
                            <View className="flex flex-row justify-center items-center gap-2">
                                ...
                                <Text className="text-sm font-rubik-bold">{ data.area } sqft</Text>
                            </View>
                        </View>
                    </View>
                                ...
                                <View className="flex flex-row flex-wrap mt-2 gap-5">
                                    {
                                        data.facilities.map((item: any, index: number) => {
                                            ...
                                            const facility: { icon: any, title: string } = parseFacility(item);
                        ...
                        data.gallery && data.gallery.length > 0 && (
                            <View className="my-5">
                                ...
                                <FlatList
                                    data={data.gallery}
                                    renderItem={({item}) => (
                                        <Image source={{ uri: item.image }} className="size-60 rounded-xl" />
                                    )}
